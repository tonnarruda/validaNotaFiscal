...
const NfValidator = () => {
  ...
  const handleCompetenciaChange = (e) => {
    ...
    setSearchCompetencia(formatted);
  };
  ...
  const executeSearch = async () => {
    ...
    try {
      ...
      setComparisonData(notasConvertidas);
        ...
        while (true) {
          ...
          parts.slice(0, -1).forEach(part => {
            ...
            try {
              ...
              console.log('JSON recebido do PDF:', nf);
  ...
  const compareWithSpreadsheet = (spreadsheetData) => {
    ...
    updatedComparisonData.forEach(item => {
      const key = item.nfCnpj !== 'N/A' 
        ? `${normalizeString(item.nfCnpj)}-${normalizeString(item.nfNumero)}`
        : `${normalizeString(item.plCnpj)}-${normalizeString(item.plNumero)}`;
  ...
  return (
    <>
      <ConfirmDialog
        ...
        title="Confirmar Nova Busca"
      >
        <p>
          Buscar uma nova competência limpará todos os dados já importados na tela (PDFs e Planilhas).
        </p>
        <p className="mt-2 font-semibold">Deseja continuar?</p>
      </ConfirmDialog>
